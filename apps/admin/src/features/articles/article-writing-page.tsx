...
import { ArrowLeft, CloudOff, HardDrive, Loader2, LogOut, Save, Trash2 } from "lucide-react";
...
function ArticleWritingPage({ admin, articleId, onLogout }: ArticleWritingPageProps) {
  ...
  useEffect(() => {
    ...
    async function loadEditorData() {
      ...
      const [taxonomyResult, articleResult] = await Promise.allSettled([
        listAdminArticleTaxonomies(),
      ...
      if (articleResult.status === "fulfilled") {
        ...
        const shouldUseLocalDraft =
          localDraft &&
          shouldRestoreArticleLocalDraft(localDraft, nextArticle?.updatedAt) &&
          !areArticleEditorValuesEqual(localDraft.values, serverValues);

        setArticle(nextArticle);
        setValues(shouldUseLocalDraft ? localDraft.values : serverValues);
  ...
  return (
    <main className="flex h-dvh min-h-0 flex-col bg-background text-foreground">
      ...
      {errorMessage || localDraftMessage || savedMessage ? (
